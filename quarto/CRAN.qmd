...
$$
L(\boldsymbol{x}) = \zeta(\alpha+1)^{-n}C_{\alpha,u}^{N-n}\prod_{i:x_i\leq u}x_i^{-(\alpha+1)}\prod_{i:x_i>u}\left\{\left(1+\frac{\xi(x_i-1)}{\sigma_u}\right)_+^{-1/\xi} - \left(1+\frac{\xi x_i}{\sigma_u}\right)_+^{-1/\xi}\right\}
$$

and,

$$
\ell(\boldsymbol{x}) = -n\log\zeta(\alpha+1)+(N-n)\log C_{\alpha,u} -(\alpha+1)\sum_{i:x_i\leq u}\log x_i + \sum_{i:x_i>u}\log\left\{\left(1+\frac{\xi(x_i-1)}{\sigma_u}\right)_+^{-1/\xi} - \left(1+\frac{\xi x_i}{\sigma_u}\right)_+^{-1/\xi}\right\}
$$
...
Where $\alpha,\beta >0$ and $\phi_u \in (0,1)$ is the excedance probability, and $\zeta(s,t) = \sum_{k=t}^\infty k^{-s}$ is the Hurwitz zeta function. From now on, we will use $\zeta_{t-1}(s)$ to denote the Hurwitz zeta function with paramters $s$ and $t$ .
...
As before we have data $\boldsymbol{x} = (x_1,x_2,\ldots,x_N)^T$ , the likelihood function for this model is as below:
...
We will be treating $\phi_u$ as a fixed value, using the MLE as its value; $\hat\phi_u = \frac{\sum_{i=1}^N I_{x_i>u}(x_i)}{N}$
